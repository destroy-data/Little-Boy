    ...
    case FETCH_DATA_LOW:
        if( !ticksInCurrentState )
            ...
            if( windowTile )
                ...
                row = ( ly + scrollY ) % 8;
            tileDataLow = mem.read( uint16_t( tileAddress + row * 2 ) );
    ...
    case FETCH_DATA_HIGH:
        if( !ticksInCurrentState )
            ...
            if( windowTile )
                ...
                row = ( ly + scrollY ) % 8;
            tileDataLow = mem.read( uint16_t( tileAddress + row * 2 + 1 ) );
            state = PUSH;
            ticksInCurrentState = 0;
        }
        break;
        ...
        if( !ppu.state.bgPixelsFifo.empty() ) {
            for( int i = 7; i >= 0; i-- ) {
                if( bgWinEnabled ) {
                    ...
                    ppu.state.bgPixelsFifo.push( Pixel( lowerBit & ( higherBit << 1 ) ) );
                } else
